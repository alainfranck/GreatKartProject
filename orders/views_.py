...
from .helper import render_pdf_view
...
def payments(request):
    if request.method == 'POST':
        #order = Order.objects.get(user=request.user, is_ordered=False, pk=request.POST.get('orderID'))
        order = Order.objects.get(is_ordered=False, pk=request.POST.get('orderID'))
...
def order_facture(request):
    ...
    try:
        ...
        pdf = render_pdf_view("orders/order_facture.html", context_dict=context)
        print(pdf.getvalue())
